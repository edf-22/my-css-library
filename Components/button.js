    ...
    parseShortcutStyle(styleString) {
      ...
      tokens.forEach(token => {
        if (token.startsWith("bg-")) {
          ...
          const value = token.slice(2);
          if (!isNaN(value)) styles.padding = value + "px";
        } else if (token.startsWith("fs-")) {
          const value = token.slice(3);
          if (!isNaN(value)) styles.fontSize = value + "px";
        }
      });
  ...
  const initButtonSystem = function () {
    ...
    window.ButtonLib = {
      ...
      init: function () {
        ...
        // Automatically apply predefined custom styles if present
        if (window.customButtonStyles && typeof window.customButtonStyles === "object") {
          for (const selector in window.customButtonStyles) {
            if (Object.hasOwn(window.customButtonStyles, selector)) {
              this.setCustomButton(selector, window.customButtonStyles[selector]);
            }
          }
        }
      },
